...
namespace ADSDKDemo
{
    public partial class frmMain : Form
    {
        ...
        public void ChangeLanguage()
        {
            switch (IniSettings.AppsLanguage)
            {
                case IniSettings.LngType.CHN:
					...
					listViewMsg.Columns[4].Text = "估算距离";
					#endregion
					break;
                default:
					...
					listViewMsg.Columns[4].Text = "Estimate Distance";
					#endregion
					break;
            }
        ...
        private void tsStatusPortOpen_MouseEnter(object sender, EventArgs e)
        {
            ...
            if (IniSettings.AppsLanguage == IniSettings.LngType.CHN)
            {
                aMsg = "读卡器当前连接状态";
            }
		...
		private void __DisplayMsgString(string s)
        {
			if (!bDisposed)
            {
                ...
                ListViewItem lvt = new ListViewItem(/*DateTime.Now.Hour.ToString("00") + ":" + DateTime.Now.Minute.ToString("00") + ":" + DateTime.Now.Second.ToString("00") + " " + DateTime.Now.Millisecond.ToString("000")*/DateTime.Now.ToString("HH:mm:ss.ffffff"));
				//DateTime dateValue = DateTime.Parse(DateTime.Now.ToString("HH:mm:ss.ffffff"));
                ...
                for (int i = 0; i < msg.Length; i++)
                {
                    if (i == 0)
                    {
                        switch (msg[0])
                        {
                            case "RCP CMD":
                                bc = Color.OldLace;
								//double tempFrom = Int32.Parse(dateValue.ToString("ffffff"));
								//dFrom = tempFrom;
								DateTime sendTime = DateTime.Now;
								dSend = sendTime.Ticks;
								break;								
                            case "RCP RSP":
                                bc = Color.Lavender;
								//double tempTo = Int32.Parse(dateValue.ToString("ffffff"));
								//dTo = tempTo;
								DateTime rspTime= DateTime.Now;
								dRsp = rspTime.Ticks;
								break;
                            case "RCP AUTO":
					...
					if (i == 0)
					{
						switch (msg[0])
						{
							...
							case "RCP RSP":
								...
								//double timeDif = (dTo - dFrom) / 1000000;
								//long timeDif = ((dRsp - dSend) *100)/ nanoSecond;
								double timeDifNano = (dRsp - dSend)*100;
								distance = (timeDifNano * 0.299792458) / 2;
								//lvt.SubItems.Add(distance.ToString());
								lvt.SubItems.Add(distance.ToString());
								//lvt.SubItems.Add(dRsp.ToString());
								break;
						}
					}
				}
