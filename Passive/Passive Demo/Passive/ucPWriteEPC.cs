...
namespace ADSDKDemo
{
    public partial class ucPWriteEPC : UserControl
    {
        ...
        {
            blnChangeFlag = true;
            IniSettings.WriteMode = cmbWiegand.SelectedIndex;
            utxtCardHex.InputMask = IniSettings.WriteMode > 0 ? "HH-HH-HH-HH" : "HH-HH-HH";
            utxtCardWg.InputMask = IniSettings.WriteMode > 0 ? "00000,00000" : "000,00000";
            RefrashCard();
            blnChangeFlag = false;
        ...
        {
            IniSettings.WritePosition = cmbPosition.SelectedIndex;
        }

        private void chkAdd_CheckedChanged(object sender, EventArgs e)
        {
            IniSettings.WriteAutoAdd = chkAdd.Checked;
        }
        ...
        private void ltxtCardDecimal_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                blnChangeFlag = true;
                LngSendData = ConvertData.DecStringToDecLong(ltxtCardDecimal.Text.Trim(), IniSettings.WriteMode>0);
        ...
        private void utxtCardHex_TextChanged(object sender, EventArgs e)
        {
            if (!blnChangeFlag)
            {
                ...
                if(chkAddHex.Checked)
                    ...
                    LngSendData = ConvertData.HexStringToDecLong(utxtCardHex.Text.Replace("-", ""), IniSettings.WriteMode > 0);
        ...
        private void utxtCardWg_TextChanged(object sender, EventArgs e)
        {
            if (!blnChangeFlag)
            {
                ...
                if (lngfirst > (IniSettings.WriteMode > 0 ? 65535 : 255)) lngfirst = (IniSettings.WriteMode > 0 ? 65535 : 255);
        ...
        {
            cmbWiegand.SelectedIndex = IniSettings.WriteMode;
            cmbPosition.SelectedIndex = IniSettings.WritePosition;
            chkAdd.Checked = IniSettings.WriteAutoAdd;
           // LngSendData = ConvertData.HexStringToDecLong(UserSettings.WriteCard.Replace("-", ""), UserSettings.WriteMode > 0);
            try
            {
                LngSendData = Convert.ToInt32(IniSettings.WriteCard);
            }
            catch { }
            RefrashCard();
        }
        ...
        public string ShowCard(long Card, bool isHex)
        {
            if (isHex)
            {
                ...
                {
                    return ConvertData.DecLongToHexString(Card, IniSettings.WriteMode > 0);
                }
            }
            else
            {
        ...
        private void GetCard(byte[] data)
        {
            if (chkAddHex.Checked)
            {
                LngRecvData = Convert.ToInt32(ConvertData.ByteArrayToHexString(data, 1 + intpoint, (IniSettings.WriteMode > 0?4:3)));
            }
            else
            {
                if (IniSettings.WriteMode > 0)
            ...
            if (LngRecvData == LngSendDataP)
            {
                ...
                IniSettings.WriteCard = LngSendDataP.ToString();
        ...
        private void ucPWriteEPC_Enter(object sender, EventArgs e)
        {
